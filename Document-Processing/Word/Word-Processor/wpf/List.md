...
The following code sample demonstrates how to retrieve the list applied to the current selection by using the [GetList](https://help.syncfusion.com/cr/wpf/Syncfusion.Windows.Controls.RichTextBoxAdv.SelectionParagraphFormat.html#Syncfusion_Windows_Controls_RichTextBoxAdv_SelectionParagraphFormat_GetList) method. If the current selection is not inside a list, the method returns `null`, so check for `null` before reusing the returned list.
...
The following code example demonstrates how to apply a list to the selection content in the SfRichTextBoxAdv control using the [SetList](https://help.syncfusion.com/cr/wpf/Syncfusion.Windows.Controls.RichTextBoxAdv.SelectionParagraphFormat.html#Syncfusion_Windows_Controls_RichTextBoxAdv_SelectionParagraphFormat_SetList_Syncfusion_Windows_Controls_RichTextBoxAdv_ListAdv_) method. When the selection content has a list, it gets modified with that list. Otherwise, the list is added to the document and applied to the selection content.
