...
documentation: ug
keywords: text-wrapping,text-wrapping-style,inline,behind,in-front-of-text,top-and-bottom,square,tight,through,wrap-mode,image-wrap,shape-wrap
---
# Text wrapping style in UWP RichTextBox

Text wrapping refers to how images and shapes are positioned relative to the surrounding text in a document. Currently, [`SfRichTextBoxAdv`](https://help.syncfusion.com/cr/uwp/Syncfusion.SfRichTextBoxAdv.SfRichTextBoxAdv.html) only has preservation support for image and text-box shapes with the following wrapping styles.
...
| `Behind` | Image or shape is placed behind the text. Useful for watermarks and background images. | Syncfusion UWP RichTextBox v18.3.0.X. |
| `In Front of Text` | Image or shape is placed in front of the text. Useful for highlighting a shape in a paragraph. | Syncfusion UWP RichTextBox v18.3.0.X. |
| `Top and Bottom` | Text wraps above and below the image or shape. No text is to the left or right. Useful for large images that occupy most of the page width. | Syncfusion UWP RichTextBox v19.1.0.X. |
| `Square` | Text wraps around the image or text box in a square shape. | Always (preservation). |
| `Tight` and `Through` | Render the same way as `Square` in SfRichTextBoxAdv. | Syncfusion UWP RichTextBox v19.2.0.X. |
...
- [Shapes in UWP RichTextBox](https://help.syncfusion.com/uwp/richtextbox/shapes)
