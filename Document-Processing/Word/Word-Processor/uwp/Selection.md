...
documentation: ug
keywords: selection,text-position,hierarchy-index,multi-selection,selection-ranges,character-format,paragraph-format,section-format,lost-focus-behavior,editing-context,delete-command
---
# Selection support in UWP RichTextBox

[`SfRichTextBoxAdv`](https://help.syncfusion.com/cr/uwp/Syncfusion.SfRichTextBoxAdv.SfRichTextBoxAdv.html) supports selecting a portion of the document either through UI interactions using the mouse, touch, or keyboard, or through the supported APIs.

N> The C# snippets in this document assume the `using Syncfusion.UI.Xaml.RichTextBoxAdv;` directive, and that the host `SfRichTextBoxAdv` is declared as `<RichTextBoxAdv:SfRichTextBoxAdv x:Name="richTextBoxAdv" />`. The samples below assume a multi-paragraph document is already loaded into the control.
...
The previous samples show how to collapse the selection to a single text position. To select a range, pass two text positions to `Selection.Select`.
...
{% tabs %}
{% highlight c# %}
// Applies page margin for the selected sections (values are in DIPs).
...
SfRichTextBoxAdv supports binding the rich-text format options of the selected content to UI elements. The following code sample demonstrates how to bind the bold format option of SfRichTextBoxAdv.
{% tabs %}
{% highlight xaml %}
...
<ToggleButton x:Name="toggleButton" Content="Left" IsChecked="{Binding ElementName=richTextBoxAdv, Path=Selection.ParagraphFormat.TextAlignment, Mode=TwoWay, Converter={StaticResource LeftAlignmentToggleConverter}}" />
...
<Button Content="Delete" Command="{Binding ElementName=richTextBoxAdv, Path=DeleteCommand}" />
...
- [Find and Replace in UWP RichTextBox](https://help.syncfusion.com/uwp/richtextbox/find-and-replace)
