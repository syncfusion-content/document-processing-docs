...
**Step 6:** Navigate to the DOCX Editor Web API at `http://<app_name>.azurewebsites.net/api/documenteditor/`. It returns the default GET-method response.

Append the running App Service URL `http://<app_name>.azurewebsites.net/api/documenteditor/` to the `serviceUrl` in the client-side DOCX Editor control. For more information about how to get started with the DOCX Editor control, refer to the [`Getting Started` page](../getting-started).
