---
title: Working with Sections in .NET Word (DocIO) library | Syncfusion<sup>®</sup>
...
N> * Whereas Essential<sup>®</sup> DocIO is a non-UI component that provides a full-fledged document object model to manipulate the Word document contents. Hence it is not feasible to set different page margins or page settings to each page instead we can do the same for each section.
