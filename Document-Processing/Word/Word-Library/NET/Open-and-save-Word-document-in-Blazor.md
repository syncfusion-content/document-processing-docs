...
Add the following code snippet to the Navigation menu's Razor file `NavMenu.razor` in the `Shared` folder.
...
![Create Blazor Web App application in Visual Studio](Blazor_Images/Blazor_images_Server_Web_App.png)
...
Step 7: Create a new `MemoryStream` method named `OpenAndSaveDocument` in the `WordService` class, and include the following code snippet to **open an existing Word document in Blazor** Web app.
