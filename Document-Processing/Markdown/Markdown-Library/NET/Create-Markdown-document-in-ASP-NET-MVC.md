...
![Install Syncfusion.Markdown NuGet package](ASP-NET-MVC_images/Install_Nuget.png)

N> Starting with v34.x.x, you also need to add the `Syncfusion.Licensing` assembly reference and register a license key. See the [Licensing](#licensing) section below for details.

Step 4: Include the following namespace in the `HomeController.cs` file. The HomeController.cs is auto-generated by the MVC template in the **Controllers** folder.
...
![ASP.NET MVC output Markdown document](ASP-NET-MVC_images/GettingStarted_Output1.png)

## Licensing

Starting with **v34.x.x**, if you reference Syncfusion<sup>&reg;</sup> assemblies from the trial setup or from the NuGet feed, you must also reference the `Syncfusion.Licensing` assembly and register a license key in your application. Register the license in `Global.asax.cs` (Application_Start) as shown below:
...
{
    // Register the Syncfusion license key obtained from your Syncfusion account.
...
To obtain a license key, refer to [Syncfusion licensing overview](https://help.syncfusion.com/common/essential-studio/licensing/overview) and [generate a license key](https://help.syncfusion.com/common/essential-studio/licensing/how-to-generate).

## Troubleshooting

If the document is not downloaded or an exception is thrown, verify the following:

* The `Syncfusion.Markdown` (and, from v34.x.x, `Syncfusion.Licensing`) NuGet packages are referenced in the project.
* A valid Syncfusion license key is registered (from v34.x.x).
* The `Data\photo.jpg` file exists under the project root, or remove the `MdPicture` block from the code sample.

## See also

* [Read a Markdown document](overview.md)
* [Convert Markdown to HTML](../Conversions/Convert-Markdown-to-HTML.md)
* [Syncfusion .NET Markdown library overview](overview.md)