...
The PDF Viewer displays existing form fields in a PDF, enables users to fill and validate them, and lets users download the filled data.
...
To disable user interaction with form fields, set `enableFormDesigner` to `false` and include `FormFieldsService` in the component providers. Example configuration:
...
* Click the signature field in the PDF to open the signature panel.
...
To remove a signature from a signature field, use the Delete option in the annotation toolbar.

![Deleting a signature in Angular PDF Viewer](./images/form-filling-signature-del.png)

## Export and import form fields

The PDF Viewer supports exporting and importing form field data using the `importFormFields`, `exportFormFields`, and `exportFormFieldsAsObject` methods. Common workflows include exporting filled data for storage or transfer and importing it later to repopulate fields. Supported formats:
