...
Create a highlight programmatically:

```cshtml
@using Syncfusion.Blazor.SfPdfViewer

<SfPdfViewer2 @ref="Viewer" DocumentPath="wwwroot/sample.pdf" Height="100%" Width="100%" />

@code {
    private SfPdfViewer2 Viewer;
    private async Task AddHighlightAsync()
    {
        var settings = new PdfAnnotationSettings
        {
            Author = "Reviewer",
            Color = "#FFFF00",
            Opacity = 0.5f
        };
        await Viewer.AddAnnotationAsync(AnnotationType.Highlight, settings);
    }
}
```
...
```cshtml
await Viewer.ImportAnnotationAsyncAsync("wwwroot/annotations.xfdf", AnnotationImportFormat.Xfdf);
```

Export annotations to JSON:

```cshtml
await Viewer.ExportAnnotationAsyncAsync("annotations.json", AnnotationExportFormat.Json);
```

Enable comments pane:

```cshtml
<SfPdfViewer2 EnableComment = "true" />
```
...
|---|---|---|---|
| `Author` | `string` | `""` | Name of the annotation author. |
| `Color` | `string` | `"#FFFF00"` | Fill or stroke color (hex). |
| `Opacity` | `float` | `1.0` | Opacity between `0.0` and `1.0`. |
| `Thickness` | `float` | `1.0` | Stroke thickness in points. |
| `Text` | `string` | `""` | Text content (for text-based annotations). |
| `IsLocked` | `bool` | `false` | Prevents editing or deletion. |
| `ModifiedDate` | `DateTime` | `DateTime.Now` | Last modification timestamp. |

## Troubleshooting

- **Annotations not persisting after refresh** — Ensure you call `ExportAnnotationAsyncAsync` before unloading, then re-import on load.
- **Toolbar buttons disabled** — Verify `AnnotationPermission` is set to `AnnotationPermission.All`.
- **Comments pane not visible** — Set `EnableComment` to `true` and ensure the user has `AnnotationPermission.Notes`.
- **Redaction not applied** — Redactions require a server-side call to the Syncfusion processing backend; see [Server-side redaction](../annotation/redaction-annotation#server-side-setup).
