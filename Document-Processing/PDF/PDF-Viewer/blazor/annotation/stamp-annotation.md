---
layout: post
title: Stamp annotations in Blazor PDF Viewer  | Syncfusion
description: Learn how to add, edit, rotate, and customize stamp annotations in the Blazor PDF Viewer component and more details.
...
If the viewer is in Pan mode when entering stamp mode, it switches to Text Select mode to enable stamp placement.

## Adding a custom stamp to the PDF document through interaction
...
N> Only JPEG image formats are supported for custom stamp annotations.
...
@code
{
    ...
    private void stampClicked(ClickEventArgs args)
    {
        pdfViewerCustomStamps = new List<PdfViewerCustomStamp>()
        {
            ...
            {
                //Specifies the custom stamp name to be added in the stamp menu of the SfPdfViewer toolbar.
                CustomStampName = "Stamp",
                //Specifies the custom stamp image source to be added in the stamp menu of the SfPdfViewer toolbar.
...
@code {
    ...
    private async Task EditStampAnnotationAsync(MouseEventArgs args)
    {
        ...
        // Select the first stamp annotation to edit
...
This code edits a stamp annotation programmatically using the SfPdfViewer component.
...
The customized templated stamp appearance feature displays user-defined stamps with tailored templates. The template controls only the visual appearance; programmatic data (bounds, opacity, author) is managed separately.
...
* [How to delete the annotation programmatically](./delete-annotation#delete-programmatically)