...
description: Learn how to create and modify annotations in ASP.NET Core PDF Viewer with UI and programmatic examples, plus quick links to all annotation types.
...
<div>
  <button id="addRedaction">Add Redaction Annotation</button>
...
{% highlight cshtml tabtitle="Standalone" %}
<div>
  <button id="bulkEdit">Bulk Edit Annotations</button>
  <ejs-pdfviewer id="container" style="height:650px" resourceUrl="https://cdn.syncfusion.com/ej2/31.2.2/dist/ej2-pdfviewer-lib" documentPath="https://cdn.syncfusion.com/content/pdf/pdf-succinctly.pdf"></ejs-pdfviewer>
</div>
<script>
window.onload = function() {
  function getViewer() { return document.getElementById('container').ej2_instances[0]; }
  document.getElementById('bulkEdit').onclick = function() {
    var viewer = getViewer();
    if (!viewer || !viewer.annotationCollection) return;
    for (var i = 0; i < viewer.annotationCollection.length; i++) {
      var ann = viewer.annotationCollection[i];
      if (ann && (ann.author === 'Guest User' || ann.subject === 'Corrections')) {
        ann.color = '#ff0000';
        ann.opacity = 0.8;
