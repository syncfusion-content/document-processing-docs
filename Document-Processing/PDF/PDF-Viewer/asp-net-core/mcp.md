---
layout: post
title: Mcp in ASP.NET Core PDF Viewer | Syncfusion
description: Set up the Syncfusion ASP.NET Core PDF Viewer MCP server so AI tools can interact with the control using a standardized protocol.
platform: document-processing
control: PDF Viewer
documentation: ug
---

# ASP.NET Core PDF Viewer MCP Server Setup
...
- **Expert PDF Viewer Knowledge** - Deep understanding of the ASP.NET Core PDF Viewer component and its implementation patterns.
...
- An **ASP.NET Core application** (existing or new); see [ASP.NET Core PDF Viewer documentation](./getting-started)
...
The examples below showcase how the `search_docs` tool handles real-world ASP.NET Core PDF Viewer development scenarios. The tool can be invoked directly, as shown in the examples below, for specific needs. Alternatively, an AI assistant can automatically invoke it based on the request.

**Get Started**

Use `search_docs` to get contextual guidance, code snippets, and configuration examples for the ASP.NET Core PDF Viewer component.

{% promptcards %}
{% promptcard Standalone Component Configuration %}
#search_docs How do I configure the Syncfusion ASP .NET Core Standalone PDF Viewer Component?
{% endpromptcard %}
{% endpromptcards %}

**Implement Features**

Get step-by-step help for adding specific features to the ASP.NET Core PDF Viewer.

{% promptcards %}
{% promptcard Form Designer %}
#search_docs How can I enable Form Designer in PDF Viewer?
{% endpromptcard %}
{% endpromptcards %}

{% promptcards %}
{% promptcard Fit to Viewport %}
#search_docs Provide API for Fit the entire page to the viewport in PDF viewer
{% endpromptcard %}
{% endpromptcards %}

{% promptcards %}
{% promptcard Programmatic Printing %}
#search_docs how to programmatically trigger printing in the PDF Viewer?
{% endpromptcard %}
{% endpromptcards %}
...
{% promptcards %}
{% promptcard Version Mismatch %}
#search_docs Document not Loading Newer version in PDF Viewer.
{% endpromptcard %}
{% endpromptcards %}

## Best Practices

To get the most out of the Syncfusion<sup style="font-size:70%">&reg;</sup> ASP.NET Core PDF Viewer MCP Server:

- **Be Specific** - Include the platform and component in your queries (for example, *"Enable the Form Designer in the ASP.NET Core PDF Viewer and load a PDF from a local file path"*).
