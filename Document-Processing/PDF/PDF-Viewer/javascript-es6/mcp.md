---
layout: post
title: Mcp in JavaScript (ES6) PDF Viewer | Syncfusion
description: Set up the Syncfusion JavaScript (ES6) PDF Viewer MCP server so AI tools can interact with the control using a standardized protocol.
...
# JavaScript (ES6) PDF Viewer MCP Server Setup
...
- **Expert PDF Viewer Knowledge** - Deep understanding of the TypeScript PDF Viewer component (SfPdfViewer) and its implementation patterns.
...
- A **TypeScript application** (existing or new); see [TypeScript PDF Viewer documentation](./getting-started)
...
The examples below showcase how the `search_docs` tool handles real-world TypeScript PDF Viewer development scenarios. The tool can be invoked directly, as shown in the examples below, for specific needs. Alternatively, an AI assistant can automatically invoke it based on the request.

**Get Started**

Use `search_docs` to get contextual guidance, code snippets, and configuration examples for the TypeScript PDF Viewer component.

{% promptcards %}
{% promptcard Standalone Component Setup %}
#search_docs How do I configure the Syncfusion TypeScript Standalone PDF Viewer Component?
{% endpromptcard %}
{% endpromptcards %}

**Implement Features**

Get step-by-step help for adding specific features to the TypeScript PDF Viewer.

{% promptcards %}
{% promptcard Form Designer %}
#search_docs How can I enable Form Designer in TypeScript PDF Viewer?
{% endpromptcard %}
{% endpromptcards %}

{% promptcards %}
{% promptcard Fit to Viewport %}
#search_docs Provide API for Fit the entire page to the viewport in TypeScript PDF Viewer.
{% endpromptcard %}
{% endpromptcards %}

{% promptcards %}
{% promptcard Programmatic Printing %}
#search_docs How to programmatically trigger printing in the TypeScript PDF Viewer?
{% endpromptcard %}
{% endpromptcards %}

{% promptcards %}
{% promptcard Retry Timeout %}
#search_docs How to Configure Retry Timeout for TypeScript PDF Viewer Requests.
{% endpromptcard %}
{% endpromptcards %}
...
#search_docs Document not Loading Newer version in TypeScript PDF Viewer.
{% endpromptcard %}
{% endpromptcards %}

## Best Practices

To get the most out of the Syncfusion<sup style="font-size:70%">&reg;</sup> TypeScript PDF Viewer MCP Server:

- **Be Specific** - Include the component and use case in your queries (for example, *"Load a PDF from a remote URL into the TypeScript PDF Viewer and enable annotation export"*).
