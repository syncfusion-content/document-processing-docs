...
The React PDF Viewer provides a comment filter dialog in the comments panel that allows you to filter annotations by the following criteria.
...
1. Open the PDF document in the React PDF Viewer
...
function App() {
  ...
  const handleApplyFilter = () => {
    if (viewerRef.current) {
      viewerRef.current.annotation.applyCommentFilter({
...
function App() {
  ...
  return (
    <div>
      <div style={{ marginBottom: '10px' }}>
        <button onClick={handleComplexFilter}>Apply Multi-Criteria Filter</button>
        <button onClick={handleClearFilter} style={{ marginLeft: '10px' }}>
          Clear Filter
        </button>
      </div>
