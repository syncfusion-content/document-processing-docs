...
- **Rendering Engine Worker** – Powered by PDFium, converts parsed PDF page data into visual output using canvas and SVG layers. Handles rasterization, text extraction, and visual rendering of PDF content with high fidelity. Perform CPU‑intensive tasks such as document parsing, font processing, and page rasterization outside the main thread. This off-thread processing ensures a responsive UI during heavy operations.
...
```js
// ✓ Correct order
import "@syncfusion/ej2-base/styles/tailwind3.css";         // Theme
import "@syncfusion/ej2-pdfviewer/styles/tailwind3.css";   // Component styles
```

N> Refer the [Theme and Styling](../theming-and-styling) UG Guide to know more about theme and styling.

**Available themes:**

- **Tailwind CSS**: Utility-first CSS framework (latest Tailwind versions supported); ideal when using utility classes and design tokens alongside component styles.
- **Material Design 3**: Modern Material Design 3 with updated colors, typography, and motion.
- **Material Design**: Classic Material Design theme with familiar UI patterns.
- **Bootstrap 5**: Bootstrap 5 theme compatibility for applications using the Bootstrap ecosystem.
- **Fluent 2**: Microsoft Fluent 2 design language support for Fluent-styled apps.
- **Fluent 2 High Contrast**: High-contrast Fluent variant for accessibility and contrast-sensitive scenarios.
