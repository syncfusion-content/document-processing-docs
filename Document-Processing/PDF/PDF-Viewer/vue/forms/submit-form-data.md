...
The Vue PDF Viewer allows submitting filled form data like text fields, checkboxes, radio buttons and dropdown values to a back end server for processing. This guide shows how to extract form data from the viewer and **post** it as `JSON` to a server endpoint.

## Prerequisites

- EJ2 Vue PDF Viewer installed and configured in your Vue app
...
1. Enable form designer in the viewer
   ...
   - Inject `FormFields` and [`FormDesigner`](https://ej2.syncfusion.com/vue/documentation/api/pdfviewer/formdesigner) services into the viewer so form APIs are available.

2. Export form data from the viewer

   - Use [`viewer.exportFormFieldsAsObject()`](https://ej2.syncfusion.com/vue/documentation/api/pdfviewer#exportformfieldsasobject) to obtain the filled values as JSON.
...
This full example shows a Vue component with the PDF viewer and a Submit button that sends form data to `/api/submit-form`.
  ...
  methods: {
    ...
    async handleSubmit() {
      try {
        ...
        const formData = await pdfviewer.exportFormFieldsAsObject();
...
- **No form values returned**: Ensure the PDF has interactive fields and the viewer has finished loading before calling [`exportFormFieldsAsObject()`](https://ej2.syncfusion.com/vue/documentation/api/pdfviewer#exportformfieldsasobject).
...
- **WASM or resource errors**: Ensure [`resourceUrl`](https://ej2.syncfusion.com/vue/documentation/api/pdfviewer#resourceurl) points to the correct Vue PDF Viewer library files.
