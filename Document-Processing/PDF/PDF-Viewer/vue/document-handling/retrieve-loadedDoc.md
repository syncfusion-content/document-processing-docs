---
layout: post
title: Retrieve the Loaded Document in Vue PDF Viewer | Syncfusion
description: Learn how to access the loaded PDF document instance in the Vue PDF Viewer using refs and the documentLoad event.
platform: document-processing
documentation: ug
domainurl: ##DomainURL##
---

# Retrieve the Loaded Document Instance in Vue PDF Viewer

This page explains how to access the Vue PDF Viewer instance using a template ref, listen for the [`documentLoad`](https://ej2.syncfusion.com/vue/documentation/api/pdfviewer/index-default#documentload) life-cycle event, and retrieve document information, page details, and metadata—so you can safely invoke viewer APIs after the PDF is loaded.

## Explanation: Why access the loaded document instance?

- The viewer instance (via **template ref**) gives you a stable handle to call APIs such as [`zoom`](https://help.syncfusion.com/document-processing/pdf/pdf-viewer/vue/magnification), [`print`](https://help.syncfusion.com/document-processing/pdf/pdf-viewer/vue/print), [`download`](https://help.syncfusion.com/document-processing/pdf/pdf-viewer/vue/download), and [`navigation`](https://help.syncfusion.com/document-processing/pdf/pdf-viewer/vue/navigation).
- The **document load event** (fires after the PDF is parsed and pages are ready) is the correct moment to read **document information** (title, author, page count, etc.) and **page metrics**, and to trigger post‑load UI logic.
- Accessing the instance too early (before load completes) may cause null/undefined errors or incomplete information.

## Reference: What you can access/call after load

After the PDF is loaded you can:

- **Read document information**: title, author, subject, keywords (metadata), page count.
- **Read page details**: total pages, current page, page size(s).
- **Call Viewer APIs** (typical examples):
  - **Zoom / Fit**: `zoomTo(125)`; fit to page/width
  - **Navigation**: go to a specific page
  - **Interactions**: enable/disable features (based on injected services)
  - **Export**: `download()`, `print()`

> Always invoke these after the `documentLoad` event fires, or from user actions that occur after load. Guard calls with optional chaining or readiness flags.

## How‑to: Get the instance with a ref and read details on load

Below is a focused snippet showing:
1) Creating a **template ref** for the viewer,
2) Wiring the **`documentLoad`** event, and
3) Reading basic **document info** and **page count**, then calling **viewer APIs** safely.

{% tabs %}
{% highlight html tabtitle="App.vue" %}
<template>
  <div style="height: 100vh">
    <ejs-pdfviewer
      ref="pdfViewer"
      id="pdfViewer"
      :documentPath="documentPath"
      :resourceUrl="resourceUrl"
      @documentLoad="onDocumentLoad"
      style="height: 100%"
    >
    </ejs-pdfviewer>
  </div>
</template>

<script>
import {
  PdfViewerComponent,
  Toolbar,
  Magnification,
  Navigation,
  Print,
} from '@syncfusion/ej2-vue-pdfviewer';

export default {
  components: {
    'ejs-pdfviewer': PdfViewerComponent,
  },
  data() {
    return {
      documentPath: 'https://cdn.syncfusion.com/content/pdf/pdf-succinctly.pdf',
      resourceUrl: 'https://cdn.syncfusion.com/ej2/31.2.2/dist/ej2-pdfviewer-lib',
    };
  },
  provide: {
    PdfViewer: [Toolbar, Magnification, Navigation, Print],
  },
  methods: {
    onDocumentLoad(args) {
      // 1) Access the component instance
      const viewer = this.$refs.pdfViewer;
      if (!viewer) return;

      // 2) Read loaded document details (shape depends on event payload/version)
      console.log('documentLoad args:', args);

      // 3) Extract document info
      const pageCount =
        (viewer && viewer.pageCount) ||
        (args && args.pageCount) ||
        '(unknown)';
      const documentName = (args && args.documentName) || '(unnamed)';
      console.log(`Loaded: ${documentName}, pages: ${pageCount}`);

      // 4) Safe API calls after load
      if (viewer && viewer.magnification) {
        viewer.magnification.zoomTo(200);
      }
      if (viewer && viewer.navigation) {
        viewer.navigation.goToPage(5);
      }
    },
  },
}
</script>
{% endhighlight %}
{% endtabs %}

**Notes**
- The event name is `documentLoad` (the callback receives load args).
- The exact event args and public methods available on the component may vary with the package version and injected services. Use `console.log(args)` once to see what's present in your build.
- Always guard calls with optional chaining (e.g., `viewer?.magnification?.zoomTo(125)`).

## Tutorial: End‑to‑End — Read metadata & call APIs after load

This example demonstrates a complete flow:
- Setting up a **template ref**
- Subscribing to `documentLoad`
- Extracting **metadata** and **pages**
- Exposing **buttons** to call viewer APIs only after load

{% tabs %}
{% highlight html tabtitle="App.vue" %}
<template>
  <div>
    <div style="margin-bottom: 12px">
      <strong>Status:</strong> {{ ready ? 'Document loaded' : 'Loading…' }}
      <br />
      <div v-if="ready">
        <div>File: {{ docInfo.name || '(unknown)' }}</div>
        <div>
          Pages:
          {{ docInfo.pageCount != null ? docInfo.pageCount : '(unknown)' }}
        </div>
        <div>Author: {{ docInfo.author || '(n/a)' }}</div>
        <div>Title: {{ docInfo.title || '(n/a)' }}</div>
      </div>
    </div>

    <div style="display: flex; gap: 8px; margin-bottom: 12px; flex-wrap: wrap">
      <button @click="zoomTo(100)" :disabled="!ready">Zoom 100%</button>
      <button @click="zoomTo(150)" :disabled="!ready">Zoom 150%</button>
      <button @click="goTo(1)" :disabled="!ready">Go to Page 1</button>
      <button @click="goTo(5)" :disabled="!ready">Go to Page 5</button>
      <button @click="printDoc" :disabled="!ready">Print</button>
      <button @click="downloadDoc" :disabled="!ready">Download</button>
    </div>

    <div style="height: 80vh">
      <ejs-pdfviewer
        ref="pdfViewer"
        id="pdfViewer"
        :documentPath="documentPath"
        :resourceUrl="resourceUrl"
        @documentLoad="handleDocumentLoad"
        style="height: 100%; width: 100%"
      >
      </ejs-pdfviewer>
    </div>
  </div>
</template>

<script>
import {
  PdfViewerComponent,
  Toolbar,
  Magnification,
  Navigation,
  Print,
  TextSelection,
  TextSearch,
} from '@syncfusion/ej2-vue-pdfviewer';

export default {
  components: {
    'ejs-pdfviewer': PdfViewerComponent,
  },
  data() {
    return {
      documentPath: 'https://cdn.syncfusion.com/content/pdf/pdf-succinctly.pdf',
      resourceUrl:
        'https://cdn.syncfusion.com/ej2/31.2.2/dist/ej2-pdfviewer-lib',
      ready: false,
      docInfo: {
        name: '',
        pageCount: undefined,
        author: '',
        title: '',
      },
    };
  },
  provide: {
    PdfViewer: [
      Toolbar,
      Magnification,
      Navigation,
      Print,
      TextSelection,
      TextSearch,
    ],
  },
  methods: {
    handleDocumentLoad(args) {
      const viewer = this.$refs.pdfViewer.ej2Instances;
      if (!viewer) return;

      console.log('documentLoad args:', args);

      this.docInfo = {
        name: args && args.documentName,
        pageCount: (viewer && viewer.pageCount) || (args && args.pageCount),
        author:
          args && args.documentInformation && args.documentInformation.author,
        title:
          args && args.documentInformation && args.documentInformation.title,
      };

      // Defer just a tick to ensure layout/modules ready before calling APIs
      this.$nextTick(() => {
        try {
          if (viewer && viewer.magnification && viewer.magnification.zoomTo) {
            viewer.magnification.zoomTo(150); // default zoom after load
          }
          if (viewer && viewer.navigation && viewer.navigation.goToPage) {
            const targetPage = 1; // keep within bounds if you want to guard
            if (
              !this.docInfo.pageCount ||
              targetPage <= this.docInfo.pageCount
            ) {
              viewer.navigation.goToPage(targetPage);
            }
          }
        } catch (e) {
          console.warn('Post-load actions failed:', e);
        } finally {
          this.ready = true;
        }
      });
    },

    // ---- UI action handlers (module APIs) ----
    zoomTo(percent) {
      const viewer = this.$refs.pdfViewer.ej2Instances;
      try {
        if (viewer && viewer.magnification && viewer.magnification.zoomTo) {
          viewer.magnification.zoomTo(percent);
        }
      } catch (e) {
        console.warn('zoomTo failed:', e);
      }
    },

    goTo(page) {
      const viewer = this.$refs.pdfViewer.ej2Instances;
      try {
        if (viewer && viewer.navigation && viewer.navigation.goToPage) {
          viewer.navigation.goToPage(page);
        }
      } catch (e) {
        console.warn('goToPage failed:', e);
      }
    },

    printDoc() {
      const viewer = this.$refs.pdfViewer.ej2Instances;
      try {
        if (viewer && viewer.print && viewer.print.print) {
          viewer.print.print();
        }
      } catch (e) {
        console.warn('print failed:', e);
      }
    },

    downloadDoc() {
      const viewer = this.$refs.pdfViewer.ej2Instances;
      try {
        if (viewer && viewer.download) {
          viewer.download();
        }
      } catch (e) {
        console.warn('download failed:', e);
      }
    },
  },
};
</script>

{% endhighlight %}
{% endtabs %}

## See also
- Vue PDF Viewer – [API Reference](https://ej2.syncfusion.com/vue/documentation/api/pdfviewer/index-default) ([methods](https://ej2.syncfusion.com/vue/documentation/api/pdfviewer/index-default#methods), [events](https://ej2.syncfusion.com/vue/documentation/api/pdfviewer/index-default#events), [properties](https://ej2.syncfusion.com/vue/documentation/api/pdfviewer/index-default#properties))
- Events: [`documentLoad`](https://ej2.syncfusion.com/vue/documentation/api/pdfviewer/index-default#documentload) and related argument shape (check your package version)
- [Modules and Services](https://help.syncfusion.com/document-processing/pdf/pdf-viewer/vue/feature-module) (e.g., Magnification, Navigation, Print) — ensure the features you call are injected
