...
Set `allowHorizontalOverflow` to `true` when the total width of the grid columns is greater than the available page width. The overflowing columns are drawn on subsequent pages while preserving the corresponding rows.
...
let columns = [
    { field: 'id', headerText: 'Employee ID', width: 100 },
    { field: 'name', headerText: 'Employee Name', width: 140 },
...
let grid: PdfGrid = new PdfGrid(source, columns);
grid.allowHorizontalOverflow = true;
grid.draw(page, { x: 10, y: 10 });
...
N> Horizontal overflow applies when the combined column width exceeds the available drawing width. When it is disabled, the grid attempts to fit its columns within the available width.
