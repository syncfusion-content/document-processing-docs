...
{% endpromptcard %}
{% promptcard FindTextInPdf, RedactPdf %}
...
{% endpromptcard %}
{% promptcard SignPdf %}
Load the vendor contract ‘vendor_agreement_draft.pdf’ from {InputDir} and apply a digital signature using the company certificate ‘certificate.pfx’ (located at {InputDir}) with the password ‘password123’. Place the signature in the bounds(500,700,100,50) of the last page and use the company logo ‘signature_logo.png’ from {InputDir} as the signature appearance image. Export the signed contract as ‘vendor_agreement_signed.pdf’ to {OutputDir}.
{% endpromptcard %}
...
{% endpromptcard %}
{% promptcard FindAndReplace, FindAndReplaceWithRegex %}
...
{% endpromptcard %}
{% promptcard CreateChart, SetChartElement, ConvertToPDF %}
...
{% endpromptcard %}
{% promptcard EncryptPresentation %}
