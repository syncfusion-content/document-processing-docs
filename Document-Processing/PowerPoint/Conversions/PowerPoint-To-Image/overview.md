...
* Support for [custom fonts registration](./NET/Custom-fonts-registration-pptx-to-image) through the `FontManager` API, enabling use of fonts that are not installed on the operating system.
...
For further information, click [here](./NET/Custom-fonts-registration-pptx-to-image).
