...
description: Learn here how to create, read, and edit Excel files in Blazor applications using .NET Excel Library.
...
Step 10: Add the following code under ``NavMenu.razor`` file present under ``Layout`` folder.
...
Step 8: Create ``Data`` folder under ``Components`` folder and add a new cs file with name as ``ExcelService`` and include the following namespaces in the file.
