...
* A **Syncfusion license key**. Register it in `Program.cs` (see the snippet in Step 4a below) or via the `SyncfusionLicense.txt` file. In Azure App Service, the recommended approach is to store the key in an environment variable or application setting and load it at startup â€” see the [Syncfusion licensing overview](https://help.syncfusion.com/common/essential-studio/licensing/overview).

## Create a simple Excel report

The steps below illustrate creating a simple invoice-formatted Excel document in Azure App Service on Windows.

Step 1: Create a new ASP.NET Core Web Application (Model-View-Controller).

![Create ASP.NET Core web application in Visual Studio](Azure-Images/App-Service-Windows/Create_Application.png)

Step 2: Name the project.

![Name the project](Azure-Images/App-Service-Windows/Name_Application_CreateExcel.png)
...
![Install Syncfusion.XlsIO.Net.Core NuGet Package](Azure-Images/App-Service-Windows/Install_NuGet_CreateExcel.png)
...
For production deployments to Azure App Service, store the key in an **App Service Application Setting** (for example, `SYNCFUSION_LICENSE_KEY`) and read it via `Environment.GetEnvironmentVariable` so the key is not committed to source control.
...
The code below uses `hostingEnvironment.WebRootPath` to resolve the logo path; if you prefer a different name, adjust the field accordingly.
...
![Hosting](Azure-Images/App-Service-Windows/Hosting_CreateExcel.png)

Step 6: Click the **Finish** button to finalize the **App Service** creation.

![App Service](Azure-Images/App-Service-Windows/App_Service_CreateExcel.png)

Step 7: Click **Close** button.

![Profile created](Azure-Images/App-Service-Windows/Profile_Created_CreateExcel.png)

Step 8: Click the **Publish** button.

![Start publish](Azure-Images/App-Service-Windows/Start_Publish_CreateExcel.png)

Step 9: The publish operation has succeeded.

![Publish has been succeeded](Azure-Images/App-Service-Windows/Publish_Success_CreateExcel.png)

Step 10: The published web page opens in the browser. 

![Browser will open after publish](Azure-Images/App-Service-Windows/CreateDocument_Button_CreateExcel.png)

Step 11: Click **Create Document** to create a simple Excel document. The output Excel document is shown below.

![Output File](Azure-Images/App-Service-Windows/CreateExcel_AppService_Windows.png)
...
The following code snippet illustrates how to read and edit an Excel file in Azure App Service on Windows.
...
N> The `application.Workbooks.Open("Sample.xlsx")` call below resolves the path relative to the app's content root. In Azure App Service on Windows, files under `wwwroot/` are served from `wwwroot/`, so the relative path works as long as the file is deployed. Alternatively, you can resolve the path via `Path.Combine(hostingEnvironment.WebRootPath, "Sample.xlsx")` as shown in the Create section.
