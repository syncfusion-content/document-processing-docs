...
Deserialization options in the Spreadsheet allow you to exclude specific features, such as styles, formats, charts, images, and text wrapping, when opening a workbook JSON object using the `openFromJson` method. By skipping unnecessary features, you can significantly improve performance, especially when working with large or complex workbooks.
