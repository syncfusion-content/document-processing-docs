...
AI Assist integrates seamlessly into your TypeScript Spreadsheet application, enabling AI-powered capabilities with minimal configuration. This section covers the required setup.
...
Add the following AI Assist dependent style references to the `~/src/styles/styles.css` file.
...
Inject the `AIAssist` module into the TypeScript Spreadsheet. This registers the AI Assist feature and makes it available in your application.

{% tabs %}
{% highlight ts tabtitle="app.ts" %}

import { Spreadsheet, AIAssist } from '@syncfusion/ej2-spreadsheet';
...
To enable `AIAssist` in the Spreadsheet component, set the [`enableAIAssist`](https://ej2.syncfusion.com/documentation/api/spreadsheet/index-default#enableaiassist) property to `true`.
...
import { Spreadsheet, AIAssist, AIAssistSettingsModel } from '@syncfusion/ej2-spreadsheet';
...
Use the [`promptRequest`](https://ej2.syncfusion.com/documentation/api/spreadsheet/index-default#promptrequest) event to add custom data — such as a user ID or session token — to the request before it reaches your server.

{% tabs %}
{% highlight ts tabtitle="app.ts" %}

import { Spreadsheet, AIAssist, AIAssistSettingsModel, PromptRequestEventArgs } from '@syncfusion/ej2-spreadsheet';
...
Use the [`promptResponse`](https://ej2.syncfusion.com/documentation/api/spreadsheet/index-default#promptresponse) event to run custom logic after the AI completes its task — for example, logging results or showing a notification.

{% tabs %}
{% highlight ts tabtitle="app.ts" %}

import { Spreadsheet, AIAssist, AIAssistSettingsModel, PromptResponseEventArgs } from '@syncfusion/ej2-spreadsheet';
...
const spreadsheet: Spreadsheet = new Spreadsheet({
    enableAIAssist: true,
    aiAssistSettings: { requestUrl: 'https://localhost:{port}/api/AIAssist/Chat' },
    promptResponse: (args: PromptResponseEventArgs) => {
...
3. Verify CORS is enabled
    * Your TypeScript app origin must be allowed in the server's CORS policy
    * Default TypeScript dev server: http://localhost:8080 (webpack-dev-server)
...
|---|---|---|
| [`promptRequest`](https://ej2.syncfusion.com/documentation/api/spreadsheet/index-default#promptrequest) | Before the prompt is sent to the server | Attach extra data or cancel the request |
| [`promptResponse`](https://ej2.syncfusion.com/documentation/api/spreadsheet/index-default#promptresponse) | After the AI completes and responds | Log results or trigger custom UI updates |
