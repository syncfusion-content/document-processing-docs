...
* Set `showGridLines` property in `sheets` as `true` or `false` to show or hide the gridlines at initial load. By default, the [showGridLines](https://ej2.syncfusion.com/angular/documentation/api/spreadsheet/sheet#showgridlines) property is enabled in each worksheet.
