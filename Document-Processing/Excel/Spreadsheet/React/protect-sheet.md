...
|`Select UnLockedCells`| Allows selection only on unlocked cells. |
...
- `setRangeReadOnly(readOnly, range, sheetIndex)` - make rows, columns, or ranges read-only
without enabling sheet protection.
