...
function App(): React.ReactElement {
  ...
  const fullDataSource = useRef<Map<number, any>>(new Map<number, any>());
  ...
  const populateRow = (rowId: number): void => {
    if (!fullDataSource.current.has(rowId)) return;

    const rowIdx = rowId + 1;
    // To insert an empty row
    spreadsheetRef.current?.insertRow([{ index: rowIdx } as any]);
  ...
  const plusIconClickHandler = () => {
    ...
    const activeCellIdx = getCellIndexes(
      (spreadsheetRef.current.getActiveSheet() as any).activeCell
    );
        ...
        content: [
          {
            ...
            click: () => {
              ...
              spreadsheetRef.current.updateCell({ template: 'plus-icon' } as any, 'A2');
