...
type Group = { startIndx: number; endIndx: number; sheetIndx: number; hidden: boolean };
type Groups = { rows: Group[]; cols: Group[] };
...
function App(): React.ReactElement {
  ...
  const insertGrpCollection = (grpCollection: any[], indx: number, model: any[]) => {
   ...
   */
  const deleteGrpCollection = (grpCollection: any[], startIndx: number, endIndx: number) => {
   ...
   */
  const toggleGroupVisibility = (groupType: 'rows' | 'cols', groupIndex: number, fromCreatedEvent: boolean) => {
    const group = groups.current[groupType][groupIndex];
    if (!fromCreatedEvent) {
      group.hidden = !group.hidden;
    }

    const sheet = getSheet(spreadsheetRef.current as any, group.sheetIndx);
  ...
  const createGroupWrapper = (groupType: 'rows' | 'cols', groupIndex: number, isHidden: boolean) => {
  ...
  return (
    <SpreadsheetComponent
      ref={(s: SpreadsheetComponent | null): void => {
