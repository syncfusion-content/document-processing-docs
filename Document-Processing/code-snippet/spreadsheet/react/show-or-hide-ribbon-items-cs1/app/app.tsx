...
function App(): React.ReactElement {
  ...
  const showInsertTab = (): void => {
    const spreadsheet = spreadsheetRef.current;
    if (!spreadsheet) return;
    (spreadsheet as any).hideRibbonTabs(['Insert'], false);
  };

  const hideHomeItems = (): void => {
    const spreadsheet = spreadsheetRef.current;
    if (!spreadsheet) return;
    spreadsheet.hideToolbarItems('Home', homeItemsToToggle);
  };

  const showHomeItems = (): void => {
    const spreadsheet = spreadsheetRef.current;
    if (!spreadsheet) return;
    (spreadsheet as any).hideToolbarItems('Home', homeItemsToToggle, false);
  };
